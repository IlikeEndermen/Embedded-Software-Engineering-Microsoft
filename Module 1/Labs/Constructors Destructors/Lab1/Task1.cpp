...
private:
   ...
   bool isActive;
   bool isValid; // It checks if the size of the function is positive, AKA Valid
   static int totalAssets; // Track total number of assets created
public:
   // Default constructor
   DigitalAsset() : fileName("untitled"), fileType("unknown"), fileSizeMB(0.0), isActive(true), isValid(true){
   ...
   // isValid(size > 0), makes the bool true if the size is positive, otherwise, makes it false
   DigitalAsset(const std::string& name, const std::string& type, double size)
      : fileName(name), fileType(type), fileSizeMB(size), isActive(true), isValid(size > 0) {
      if (isValid){
      auto now = std::chrono::system_clock::now();
      auto time_t = std::chrono::system_clock::to_time_t(now);
      creationDate = std::ctime(&time_t);
      ...
      std::cout << "Asset created: " << fileName << " (" << fileType << ") | Total assets: " << totalAssets << std::endl;
         ...
         std::cout << "Size of " << fileName << " is negative, meaning this file can't exist" << '\n';
      }
   }   
   // Copy constructor
...
int main() {
   ...
   DigitalAsset negative_video("testing_video.mp4", "video", -150.0);
   logo.displayInfo();
   video.displayInfo();
   std::cout << std::endl;   
