...
int main() {
    ...
    vector<unique_ptr<ServerConfig>> servers;
    int counter = 0;
    ...
    servers.push_back(std::move(webConfig));
    counter++;
    auto dbConfig = createServerConfig("DatabaseServer-01", 5432, "Production");    

    //When moving into the vector, the dbConfig does become a nullptr, so I can't use it
    // in the takeOwnership code
    servers.push_back(std::move(dbConfig));
    counter++;
    // Create managers
    ...
    webManager.takeOwnership(std::move(servers[servers.size() - (counter - 1)]));
    counter--;
    dbManager.takeOwnership(std::move(servers[servers.size() - (counter - 1)]));
    counter--;
